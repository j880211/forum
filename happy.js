...
function init() {
    ...
    post_btn.addEventListener('click', function () {
        if (post_txt.value != "") {
                ...
                firebase.database().ref('happy_list').push({
    ...
    var postsRef = firebase.database().ref('happy_list');
    var total_post = [];
    ...
    postsRef.once('value')
        .then(function (snapshot) {
            ...
            for (var key in object)
            {
                    ...
                    document.getElementById('post_list').innerHTML += str_before_username_post + object[key].title + blank + object[key].username + str_after_content_post + key +button;
            ...
            postsRef.on('value',function(snapshot)
            {
                ...
                for (var key in newobject)
                {
                        ...
                        document.getElementById('post_list').innerHTML += str_before_username_post + newobject[key].title + blank + newobject[key].username + str_after_content_post + key +button
                }
            })
        })
        ...
        happy.on('child_added', function(data){
            if(isNotify){
                isNotify=-1;
                if(Notification.permission!='default'){
                    var notify;
                    notify=new Notification('New topic from Happy', {
                        'User: ':data.val().email,
