...
function init() {
        ...
        if (user) {
            ...
            btnlogout.addEventListener('click', async function () {
                firebase.auth().signOut().then(function() {
                    // Sign-out successful.
                    alert("log out success"); // print nothing
                  }, function(error) {
                    // An error happened.
                    alert("log out error"); // print nothing
                  });
    ...
    var postsRef = firebase.database().ref('angry_list');
    ...
    postsRef.once('value')
        ...
        angry.on('child_added', function(data){
            if(isNotify){
                isNotify=-1;
                if(Notification.permission!='default'){
                    var notify;
                    notify=new Notification('New topic from Angry', {
                        'User: ':data.val().email,
...
function changeboard(i)
{
    ...
    window.location.href="angry_comment.html?"+i;
}
