...
function init() {
    ...
    post_btn.addEventListener('click', function () {
        if (post_txt.value != "") {
                firebase.database().ref('cry_list').push({
    ...
    var postsRef = firebase.database().ref('cry_list');
    ...
    postsRef.once('value')
        ...
        var cry=firebase.database().ref('cry_list');
         cry.on('child_added', function(data){
            if(isNotify){
                isNotify=-1;
                if(Notification.permission!='default'){
                    var notify;
                    notify=new Notification('New topic from Cry', {
                        'User: ':data.val().email,
...
function changeboard(i)
{
    ...
    window.location.href="cry_comment.html?"+i;
}
